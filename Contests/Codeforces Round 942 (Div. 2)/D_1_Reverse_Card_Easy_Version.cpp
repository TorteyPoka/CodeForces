...
void solve()
{
    int n, m;
    cin >> n >> m;
    vector<int> preCalGCD(2000000);
    vector<int> plusVal(1999999);
    ...
    //     for(int j = 1; j <= m; j++)
    //     {
    //         preCalGCD[k] = j*(__gcd(i,j));
    //         k++;
    //     }
    // }

    // for(auto x : preCalGCD)cout << x << " ";
    ...
    // for(int i = 0; i<plusVal.size(); i++)
    // {
    //     for(int j = 0; j < preCalGCD.size(); j++)
    //     {
    //         if(preCalGCD[j] % plusVal[i] == 0)
    //         {
    //             cnt++;
    //         }
    //     }
