...
//---------------------Sum of Digit Sum  within a Range-------------------//

#define digit_sum_range 2000000

int digit_sum_within_a_range[digit_sum_range + 1];

void digitSumWithin()
{
    for (int i = 1; i <= digit_sum_range; i++)
    {
        int c = i;
        while (c > 0)
            digit_sum_within_a_range[i] += c % 10, c /= 10;
    }
    for (int i = 1; i <= digit_sum_range; i++)
        digit_sum_within_a_range[i] += digit_sum_within_a_range[i - 1];
}
...
signed main()
{
    ...
    while (T--)
    {
        int n;
        cin >> n;
        cout << digit_sum_within_a_range[n] << endl;
    }
    return 0;
}